...
var assets = {
  ...
  button: "openDataContext/assets/button2.png",
  button2: "openDataContext/assets/button1.png",
...
function changeTotalGroup(parme) {
  let totalGroup = [];
  parme.map((val, index)=>{
    let obj = {};
    obj['key'] = index+1;
    ...
    obj['scroes'] = val.KVDataList[0].value
    totalGroup.push(obj);
  })
  return totalGroup;
}
...
function drawButton() {
  ...
  context.drawImage(assets.button2, lastButtonX, lastButtonY, buttonWidth, buttonHeight);
}
...
function drawByData(data, i) {
  ...
  img.src = data.url;
  img.onload = ()=>{
    context.drawImage(img, x, startY + i * preOffsetY + (barHeight - avatarSize) / 2, avatarSize, avatarSize);
